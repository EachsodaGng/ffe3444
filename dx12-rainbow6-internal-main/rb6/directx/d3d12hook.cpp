...
namespace d3d12hook {
    ...
    long __fastcall hookPresentD3D12(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags) {
        ...
        if (GetAsyncKeyState(VK_INSERT) & 0x1) {
			show_menu = !show_menu; // <-- Toggles Menu
        }
        ...
        if (!init) {
            if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D12Device), (void**)&d3d12Device))) {
                ...
                ImFont* Default = io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Arial.ttf", 16);
        ...
        if (shutdown == false) {
            ...
            if (show_menu) {
                Render::user_interface( ); // <-- Renders Interface
            }



			rainbow6::Run( ); // <-- Runs Cheat Logic
